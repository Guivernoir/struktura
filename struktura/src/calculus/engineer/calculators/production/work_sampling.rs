...
mod tests {
    ...
    #[tokio::test]
    async fn test_work_sampling() {
        let calc = WorkSamplingCalculator;
        
        let mut params = minimal_parameters();
        let mut additional = HashMap::new();
        additional.insert("total_observations".to_string(), 500.0);
        additional.insert("productive_observations".to_string(), 400.0);
        additional.insert("confidence_level".to_string(), 95.0);
    ...
    #[test]
    fn test_excess_productive_obs() {
        let calc = WorkSamplingCalculator;
        
        let mut params = minimal_parameters();
        let mut additional = HashMap::new();
        additional.insert("total_observations".to_string(), 500.0);
        additional.insert("productive_observations".to_string(), 600.0); // Invalid
        params.additional = Some(additional);
