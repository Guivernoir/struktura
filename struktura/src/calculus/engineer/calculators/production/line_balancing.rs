...
mod tests {
    ...
    #[tokio::test]
    async fn test_line_balancing_basic() {
        let calc = ProductionLineBalancingCalculator;
        
        let mut params = minimal_parameters();
        let mut additional = HashMap::new();
        additional.insert("num_workstations".to_string(), 5.0);
        additional.insert("desired_output_per_day".to_string(), 480.0);
        additional.insert("available_time_per_day".to_string(), 480.0);
        additional.insert("total_task_time".to_string(), 12.0);
        additional.insert("longest_task_time".to_string(), 3.5);
    ...
    #[test]
    fn test_impossible_demand() {
        let calc = ProductionLineBalancingCalculator;
        
        let mut params = minimal_parameters();
        let mut additional = HashMap::new();
        additional.insert("desired_output_per_day".to_string(), 1000.0); // Too high
        additional.insert("available_time_per_day".to_string(), 480.0);
        additional.insert("total_task_time".to_string(), 10.0); // Can't do 1000 units
        params.additional = Some(additional);
