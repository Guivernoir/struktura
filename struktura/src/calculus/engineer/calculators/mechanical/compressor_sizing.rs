...
mod tests {
    ...
    #[tokio::test]
    async fn test_compressor_sizing() {
        let calc = CompressorSizingCalculator;
        
        let mut params = minimal_parameters();
        let mut additional = HashMap::new();
        additional.insert("p_in".to_string(), 101.325);
        additional.insert("p_out".to_string(), 500.0);
        additional.insert("flow_rate".to_string(), 10.0);
        additional.insert("gas_constant".to_string(), 287.0);
        additional.insert("k".to_string(), 1.4);
        additional.insert("efficiency".to_string(), 75.0);
    ...
    fn test_invalid_pressure() {
        ...
        let mut params = minimal_parameters();
        let mut additional = HashMap::new();
        additional.insert("p_in".to_string(), 500.0);
        additional.insert("p_out".to_string(), 101.325); // Invalid
        params.additional = Some(additional);
