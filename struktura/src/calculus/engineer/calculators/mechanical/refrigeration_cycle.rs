...
mod tests {
    ...
    #[tokio::test]
    async fn test_refrigeration_cycle() {
        let calc = RefrigerationCycleCalculator;
        
        let mut params = minimal_parameters();
        let mut additional = HashMap::new();
        additional.insert("t_evap".to_string(), -10.0);
        additional.insert("t_cond".to_string(), 40.0);
        additional.insert("cooling_capacity".to_string(), 100.0);
        additional.insert("isentropic_eff".to_string(), 80.0);
    ...
    #[test]
    fn test_invalid_temps() {
        let calc = RefrigerationCycleCalculator;
        
        let mut params = minimal_parameters();
        let mut additional = HashMap::new();
        additional.insert("t_evap".to_string(), 40.0);
        additional.insert("t_cond".to_string(), 30.0); // Invalid
        params.additional = Some(additional);
